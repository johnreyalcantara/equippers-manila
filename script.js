...
/* ══════════════════════════════════════════════
   ...
   landing page becomes visible. Fetches from the
   public API so the verse matches what logged-in
   users see on the dashboard (DB-first, then
   fallback to the hardcoded list on the server).
   ══════════════════════════════════════════════ */
...
(function () {
  ...
  function initVersePopup() {
    ...
    // Fetch today's verse from the public API
    fetch('/api/verses/public/today')
      .then(function (res) { return res.json(); })
      .then(function (data) {
        textEl.textContent = data.verse;
        refEl.textContent = '\u2014 ' + data.reference;
      })
      .catch(function () {
        textEl.textContent = 'For I know the plans I have for you, declares the Lord, plans to prosper you and not to harm you, plans to give you hope and a future.';
        refEl.textContent = '\u2014 Jeremiah 29:11';
      });
